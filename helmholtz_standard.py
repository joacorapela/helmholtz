...
class HelmholtzMachine(nn.Module):
    ...
    def __init__(self, layers=[784, 128, 32, 10]):
...
def main():
    ...
    # arrays for keeping track of error
    steps_save = []
    loss_save  = []
    ...
    for step in range(iterations):
        ...
        if step % log_every == 0:
            ...
            sys.stdout.write("\n")
            print("\tgeneration_bias_loss", generation_bias_loss)
            for i, loss in enumerate(generative_loss):
                print("\tgenerative loss", i, loss)
            for i, loss in enumerate(recognition_loss):
                print("\trecognition loss", i, loss)
            sys.stdout.write("\n")
    ...
    # plot input
    plt.clf()
    printImageD(aBatch, filename="figd/inputImage.png")

    # plot generative output
    plt.clf()
    printImageD(generative_outputs[-1], filename="figd/generativeOutput.png")

    # plot recognition
    plt.clf()
    printLatent(recognition_outputs[-1], filename="figd/recognitionOutput.png")
...
def printImageD(aBatchImages, filename=None):
    ...
    for i in range(0, nplots):
        ix = i % nx
        iy = int(i/ny)
        # anArray = aBatchImages[i,:].numpy()
        # anImage = numpy.reshape(anArray, (28, 28))
        # axs[ix, iy].imshow(anImage)
        axs[ix, iy].imshow(numpy.reshape(aBatchImages[i,:].numpy(), (28, 28)))

    # old command: plt.imshow(anImage)
...
def printLatent(aBatchLatents, marker='+', filename=None):
    ...
    for i in range(0, nplots):
        ix = i % nx
        iy = int(i/ny)
        # anArray = aBatchLatents[i,:].numpy()
        axs[ix, iy].plot(aBatchLatents[i,:].numpy(), marker)

    # old command: plt.plot(anArray, marker)
...
def printLoss(steps_save, loss_save, filename=None):
    # plot
    plt.ylim([0, 1+int(max(loss_save))])
    plt.plot(steps_save, loss_save)
